...
class game_state:
    ...
    def __init__(self):
        ...
        if DIMENSION_COL == 4:
            self.init_4x4()
        elif DIMENSION_COL == 6:
            self.init_6x6()
        elif DIMENSION_COL == 8:
            self.init_8x8()
        else:
            print("error")
        
    def init_4x4(self):
        self._black_king_location = [0, 2]
        self._white_king_location = [DIMENSION_ROW - 1, 2]

        self.black_king_can_castle = [True, True,
                                      True]  # Has king not moved, has Rook1(col=0) not moved, has Rook2(col=7) not moved
        self.white_king_can_castle = [True, True, True]

        black_rook_1 = Knight('n', 0, 0, Player.PLAYER_2)
        black_rook_2 = Bishop('b', 0, 3, Player.PLAYER_2)
        black_queen = Rook('r', 0, 1, Player.PLAYER_2)
        black_king = King('k', 0, 2, Player.PLAYER_2)
        black_pawn_1 = Pawn('p', 1, 0, Player.PLAYER_2)
        black_pawn_2 = Pawn('p', 1, 1, Player.PLAYER_2)
        black_pawn_3 = Pawn('p', 1, 2, Player.PLAYER_2)
        black_pawn_4 = Pawn('p', 1, 3, Player.PLAYER_2)

        self.black_pieces = [black_rook_1, black_rook_2, black_queen, black_king, black_pawn_1, black_pawn_2, black_pawn_3, black_pawn_4]

        pawn_row = DIMENSION_ROW - 2
        king_row = DIMENSION_ROW - 1

        white_rook_1 = Knight('n', king_row, 0, Player.PLAYER_1)
        white_rook_2 = Bishop('b', king_row, 3, Player.PLAYER_1)
        white_queen = Rook('r', king_row, 1, Player.PLAYER_1)
        white_king = King('k', king_row, 2, Player.PLAYER_1)
        white_pawn_1 = Pawn('p', pawn_row, 0, Player.PLAYER_1)
        white_pawn_2 = Pawn('p', pawn_row, 1, Player.PLAYER_1)
        white_pawn_3 = Pawn('p', pawn_row, 2, Player.PLAYER_1)
        white_pawn_4 = Pawn('p', pawn_row, 3, Player.PLAYER_1)

        self.white_pieces = [white_rook_1, white_rook_2, white_queen, white_king, white_pawn_1, white_pawn_2, white_pawn_3, white_pawn_4]


        if DIMENSION_ROW < 4:
            print("ERROR")
        else:
            empty = DIMENSION_ROW - 4

            self.board = [
            [black_rook_1, black_queen, black_king, black_rook_2],
            [black_pawn_1, black_pawn_2, black_pawn_3, black_pawn_4],
            [white_pawn_1, white_pawn_2, white_pawn_3, white_pawn_4],
            [white_rook_1, white_queen, white_king, white_rook_2]
            ]

            while empty > 0:
                row = [Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY]
    ...
    def init_6x6(self):
        ...
        self._white_king_location = [DIMENSION_ROW - 1, 3]
        ...
        black_rook_2 = Rook('r', 0, 5, Player.PLAYER_2)
        black_knight_1 = Knight('n', 0, 1, Player.PLAYER_2)
        black_knight_2 = Knight('n', 0, 4, Player.PLAYER_2)
        black_queen = Queen('q', 0, 2, Player.PLAYER_2)
        black_king = King('k', 0, 3, Player.PLAYER_2)
        black_pawn_1 = Pawn('p', 1, 0, Player.PLAYER_2)
        black_pawn_2 = Pawn('p', 1, 1, Player.PLAYER_2)
        black_pawn_3 = Pawn('p', 1, 2, Player.PLAYER_2)
        black_pawn_4 = Pawn('p', 1, 3, Player.PLAYER_2)
        black_pawn_5 = Pawn('p', 1, 4, Player.PLAYER_2)
        black_pawn_6 = Pawn('p', 1, 5, Player.PLAYER_2)

        self.black_pieces = [black_rook_1, black_rook_2, black_knight_1, black_knight_2, black_queen, black_king, black_pawn_1, black_pawn_2, black_pawn_3, black_pawn_4, black_pawn_5, black_pawn_6]

        pawn_row = DIMENSION_ROW - 2
        king_row = DIMENSION_ROW - 1

        white_rook_1 = Rook('r', king_row, 0, Player.PLAYER_1)
        white_rook_2 = Rook('r', king_row, 5, Player.PLAYER_1)
        white_knight_1 = Knight('n', king_row, 1, Player.PLAYER_1)
        white_knight_2 = Knight('n', king_row, 4, Player.PLAYER_1)
        white_queen = Queen('q', king_row, 2, Player.PLAYER_1)
        white_king = King('k', king_row, 3, Player.PLAYER_1)
        white_pawn_1 = Pawn('p', pawn_row, 0, Player.PLAYER_1)
        white_pawn_2 = Pawn('p', pawn_row, 1, Player.PLAYER_1)
        white_pawn_3 = Pawn('p', pawn_row, 2, Player.PLAYER_1)
        white_pawn_4 = Pawn('p', pawn_row, 3, Player.PLAYER_1)
        white_pawn_5 = Pawn('p', pawn_row, 4, Player.PLAYER_1)
        white_pawn_6 = Pawn('p', pawn_row, 5, Player.PLAYER_1)

        self.white_pieces = [white_rook_1, white_rook_2, white_knight_1, white_knight_2, white_queen, white_king, white_pawn_1, white_pawn_2, white_pawn_3, white_pawn_4, white_pawn_5, white_pawn_6]

        if DIMENSION_ROW < 4:
            print("ERROR")
        else:
            self.board = [
                [black_rook_1, black_knight_1, black_queen, black_king, black_knight_2, black_rook_2],
                [black_pawn_1, black_pawn_2, black_pawn_3, black_pawn_4, black_pawn_5, black_pawn_6],
                [white_pawn_1, white_pawn_2, white_pawn_3, white_pawn_4, white_pawn_5, white_pawn_6],
                [white_rook_1, white_knight_1, white_queen, white_king, white_knight_2, white_rook_2]
            ]
            
            empty = DIMENSION_ROW - 4

            while empty > 0:
                row = [Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY]
                self.board.insert(2, row)
                empty = empty - 1
    ...
    def init_8x8(self):
        ...
        black_rook_1 = Rook('r', 0, 0, Player.PLAYER_2)
        black_rook_2 = Rook('r', 0, 7, Player.PLAYER_2)
        black_knight_1 = Knight('n', 0, 1, Player.PLAYER_2)
        black_knight_2 = Knight('n', 0, 6, Player.PLAYER_2)
        black_bishop_1 = Bishop('b', 0, 2, Player.PLAYER_2)
        black_bishop_2 = Bishop('b', 0, 5, Player.PLAYER_2)
        ...
        black_pawn_5 = Pawn('p', 1, 4, Player.PLAYER_2)
        black_pawn_6 = Pawn('p', 1, 5, Player.PLAYER_2)
        black_pawn_7 = Pawn('p', 1, 6, Player.PLAYER_2)
        black_pawn_8 = Pawn('p', 1, 7, Player.PLAYER_2)
        ...
        white_rook_1 = Rook('r', king_row, 0, Player.PLAYER_1)
        white_rook_2 = Rook('r', king_row, 7, Player.PLAYER_1)
        white_knight_1 = Knight('n', king_row, 1, Player.PLAYER_1)
        white_knight_2 = Knight('n', king_row, 6, Player.PLAYER_1)
        white_bishop_1 = Bishop('b', king_row, 2, Player.PLAYER_1)
        white_bishop_2 = Bishop('b', king_row, 5, Player.PLAYER_1)
        white_queen = Queen('q', king_row, 4, Player.PLAYER_1)
        white_king = King('k', king_row, 3, Player.PLAYER_1)
        white_pawn_1 = Pawn('p', pawn_row, 0, Player.PLAYER_1)
        white_pawn_2 = Pawn('p', pawn_row, 1, Player.PLAYER_1)
        white_pawn_3 = Pawn('p', pawn_row, 2, Player.PLAYER_1)
        ...
        white_pawn_5 = Pawn('p', pawn_row, 4, Player.PLAYER_1)
        white_pawn_6 = Pawn('p', pawn_row, 5, Player.PLAYER_1)
        white_pawn_7 = Pawn('p', pawn_row, 6, Player.PLAYER_1)
        white_pawn_8 = Pawn('p', pawn_row, 7, Player.PLAYER_1)
        ...
        if DIMENSION_ROW < 4:
            ...
            while empty > 0:
                row = [Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY, Player.EMPTY]
                self.board.insert(2, row)
                empty = empty - 1 
    ...
    def get_valid_moves(self, starting_square):
        ...
        if self.is_valid_piece(current_row, current_col):
            ...
            if self.get_piece(current_row, current_col).is_player(Player.PLAYER_2):
    ...
    def king_can_castle_left(self, player):
        ...
        if player is Player.PLAYER_2:
            ...
            return self.white_king_can_castle[0] and self.white_king_can_castle[1] and \
                   self.get_piece(DIMENSION_ROW - 1, 1) is Player.EMPTY and self.get_piece(DIMENSION_ROW - 1, 2) is Player.EMPTY and not self._is_check
    ...
    def king_can_castle_right(self, player):
        ...
        if player is Player.PLAYER_2:
            ...
            return self.white_king_can_castle[0] and self.white_king_can_castle[2] and \
                   self.get_piece(DIMENSION_ROW - 1, 6) is Player.EMPTY and self.get_piece(DIMENSION_ROW - 1, 5) is Player.EMPTY and not self._is_check
    ...
    def move_piece(self, starting_square, ending_square, is_ai):
        ...
        if self.is_valid_piece(current_square_row, current_square_col) and \
            ...
            if ending_square in valid_moves:
                ...
                if moving_piece.get_name() is "k":
                    if moving_piece.is_player(Player.PLAYER_2):
                    ...
                    if moving_piece.is_player(Player.PLAYER_2) and current_square_col == 0:
                        self.black_king_can_castle[1] = False
                    elif moving_piece.is_player(Player.PLAYER_2) and current_square_col == 7:
                        self.black_king_can_castle[2] = False
                    elif moving_piece.is_player(Player.PLAYER_1) and current_square_col == 0:
                        self.black_king_can_castle[1] = False
                ...
                # Add move class here
                    ...
                    if moving_piece.is_player(Player.PLAYER_2) and next_square_row == DIMENSION_ROW - 1:
                    ...
                    elif moving_piece.is_player(Player.PLAYER_1) and next_square_row == 0:
    ...
    def undo_move(self):
        if self.move_log:
            ...
            if undoing_move.castled is True:
                ...
                if undoing_move.moving_piece is Player.PLAYER_2:
            ...
            #     self._is_check = True
            if undoing_move.moving_piece.get_name() is 'k' and undoing_move.moving_piece.get_player() is Player.PLAYER_2:
                self._black_king_location = (undoing_move.starting_square_row, undoing_move.starting_square_col)
            elif undoing_move.moving_piece.get_name() is 'k' and undoing_move.moving_piece.get_player() is Player.PLAYER_1:
